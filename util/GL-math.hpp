...
namespace GL { template <typename T> classname normalize(classname vec) { return vec / length(vec); } }
...
namespace GL { bool funcname(classname vec) { bool ret = vec[0]; for (int i = 1; i < numelements; i++) ret = ret op vec[i]; return ret; } }

#define _MakeBoolFunctions(funcname, op) \
_MakeBoolFunction(funcname, bvec2, 2, op) \
_MakeBoolFunction(funcname, bvec3, 3, op) \
_MakeBoolFunction(funcname, bvec4, 4, op)
...
namespace GL {
	...
	vec3 cross(vec3 x, vec3 y) { return vec3(x.y * y.z - x.z * y.y, x.z * y.x - x.x * y.z, x.x * y.y - x.y * y.x); }
	dvec3 cross(dvec3 x,dvec3 y) { return dvec3(x.y * y.z - x.z * y.y, x.z * y.x - x.x * y.z, x.x * y.y - x.y * y.x); }
